...
函数签名生成参数 schema，docstring 告诉模型工具的用途。命令执行、输出截断、超时和基础危险命令拦截都直接放在同一个工具函数中，便于第一章完整阅读，langchain中使用@tool简易封装工具。
...
这一行会在 LangGraph 上创建预构建执行图。它完成参考实现中的关键闭环：调用模型、识别工具调用、执行 Bash、自动将结果作为 `ToolMessage` 放回状态，再次调用模型，直到模型给出最终回答。
