# s16: Workflow Runtime — 编排固定时写进代码

> **状态：尚未实现。** 本章目录用于保持与参考仓库一致的 17 章编排。

[s15](../s15_integrated_harness/) → **s16** → [s17](../s17_goal_loop/)

当前 `code.py` 是 0 字节占位文件。等 LangChain 版本完成后，再补充教学 README；目前不复制参考仓库的 Python 实现，以免把 Anthropic SDK 版本误认为本项目代码。

参考章节：[`shareAI-lab/learn-claude-code/s16_workflow_runtime`](https://github.com/shareAI-lab/learn-claude-code/tree/main/s16_workflow_runtime)
